...
@Client.on_message(command(["مبرمج السورس", f"صلاح حمدان", f"صلاح سوريا", f"السورس", f"مطور", f"المطور"]) & filters.group & ~filters.edited)
async def start(client: Client, message: Message):
    await message.reply_photo(
        photo=f"https://telegra.ph/file/872bf4505dc784b128c3e.jpg",
        caption=f"""⌔ قناة سورس الماس [Source Diamond](https://t.me/CR_T2)\n⌔ افضل سورس ميوزك على التليجرام\n⌔ انضم للقناة لمتابعة تحديثات السورس""",
        reply_markup=InlineKeyboardMarkup(
         [
            [
...
async def help(client: Client, message: Message):
    await message.reply_photo(
        photo=f"{DEV_PHOTO}",
        caption=f"""⌔ Source Diamond Music 💎""",
        reply_markup=InlineKeyboardMarkup(
         [
            [
...
@Client.on_message(command(["بينج", f"بنك"]) & ~filters.edited)
async def ping_pong(client: Client, message: Message):
    start = time()
    m_reply = await message.reply_text("⏳ جاري فحص سرعة البوت...")
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            ...
                            InlineKeyboardButton("‹ جروب لدعم ›", url=f"https://t.me/{GROUP_SUPPORT}")
                        ],
                        [
                            InlineKeyboardButton(
